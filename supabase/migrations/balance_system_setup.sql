-- Balance System Setup for Split Smart App
-- Run this in your Supabase SQL Editor to set up the balance system
...
CREATE TABLE IF NOT EXISTS public.default_balance_titles (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Insert default balance titles (only if they don't exist)
...
DROP POLICY IF EXISTS "All users can view default balance titles" ON public.default_balance_titles;
...
CREATE INDEX IF NOT EXISTS balance_transactions_type_idx ON public.balance_transactions(transaction_type);
CREATE INDEX IF NOT EXISTS balance_transactions_created_at_idx ON public.balance_transactions(created_at);
CREATE INDEX IF NOT EXISTS balance_transactions_expense_share_id_idx ON public.balance_transactions(expense_share_id);
CREATE INDEX IF NOT EXISTS balance_transactions_group_id_idx ON public.balance_transactions(group_id);

-- Drop existing functions if they exist
DROP FUNCTION IF EXISTS create_user_balance() CASCADE;
...
BEGIN
  -- Get current balance
  SELECT current_balance, total_added, total_spent, total_loans, total_repaid
...
-- Success message
SELECT 'Balance system setup completed successfully!' as status; 