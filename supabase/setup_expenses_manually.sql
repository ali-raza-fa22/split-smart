-- Manual setup for expenses tables
...
-- 2. Create expenses table with correct foreign keys
...
-- 4. Add expense_data column to group_messages table
...
-- 7. Create policies for expense_shares
...
CREATE INDEX group_messages_expense_data_idx ON public.group_messages USING GIN (expense_data);

-- 9. Create function to automatically create expense shares
...
-- 10. Create trigger to automatically create expense shares
...
SELECT COUNT(*) as expense_shares_count FROM public.expense_shares;
SELECT 'expense_data column exists' as column_check FROM information_schema.columns 
